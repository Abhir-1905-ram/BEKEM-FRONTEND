...
export function PMBranchTransferApprovalsPage() {
  const navigate = useNavigate();

  const { data: transfers, list } = useListQuery({
    queryKey: ['pm-branch-transfer-approvals'],
    queryFn: async () => {
      const res = await api.get<{ data: BranchTransferDto[] }>('/branch-transfers');
      return normalizeListData<BranchTransferDto>(res.data.data).filter(
        (t) => t.status === 'REQUESTED' && t.canPmApprove
      );
    },
  });
  ...
  return (
    <div className="page-container max-w-2xl">
      <PageHeader
        ...
        subtitle="Open a request to review details, then approve or reject"
        action={
          <button
            type="button"
            onClick={() => navigate('/')}
      ...
      <ListQueryBoundary
        ...
        skeletonRows={2}
        empty={<EmptyState title="No pending branch transfers" description="You're all caught up." />}
      >
